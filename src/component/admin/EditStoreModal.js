...
export default function EditStoreModal({ store, isOpen, onClose, refresh }) {
    ...
    const previewUrl = useMemo(() => file ? URL.createObjectURL(file) : image, [file])
    ...
    async function onFormSubmit(e) {
        ...
        try {
            setLoading(true)
            const { data } = await api.patch(`/restaurants/${id}`, { name: nameInput, address: addressInput, description: descriptionInput })
            if (file) {
                const formData = new FormData();
                formData.append('file', file);
                await api.post(`/restaurants/${id}/image`, formData, {
                    headers: {
